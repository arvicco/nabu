...
module Nabu
  module Adapters
    ...
    class Proiel < Nabu::Adapter
      ...
      # Clone or non-destructively pull the single upstream repo into
      # +workdir+ via the shared git path (Adapter#git_fetch! → Nabu::GitFetch,
      # P5-2: attic + pre-merge mass-deletion breaker), returning a
      # Nabu::FetchReport pinning HEAD. No network in tests: exercised against
      # a local fixture git repo. A Shell failure aborts the sync as
      # Nabu::FetchError; a tripped breaker as Nabu::SyncAborted (+force+
      # overrides).
