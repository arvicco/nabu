...
module Nabu
  module Store
    module AxisBuilder
      # TOROT chronicle annals (P16-3, part 2 of the date/place axis) — the
      # first PASSAGE-GRAIN axis rows, the use the nullable passage_seq_*
      ...
      # Instruction") is skipped and counted. AM → CE via DateAxis.am_to_ce
      ...
      module ChronicleAnnals
        ...
        # One TOROT source file → its axis rows, or nil when it is not an
        ...
        def parse_am_title(title)
          ...
          not_before, not_after = DateAxis.am_to_ce(am_lo, am_hi)
        ...
        def scan_divs(path)
          ...
          [] # a malformed source is the loader's problem to report, not the axis pass's
        end
