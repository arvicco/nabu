...
module Nabu
  module Store
    module AxisBuilder
      # CDLI catalog dates + proveniences → the date/place axis (P31-2).
      #
      ...
      module CdliDates
        ...
        def each_catalog_row(path, &)
          ...
          nil # a torn catalog costs the axis, never the rebuild
        end
        ...
        def insert_row(catalog, document_id, row, counts)
          ...
          counts[:documents] += 1
          axis = date || { not_before: nil, not_after: nil, precision: nil, date_raw: nil }
          AxisBuilder.insert_axis(
            catalog, document_id, axis.merge(place_name: place, place_ref: nil), CDLI_SLUG
          )
        end
        ...
        # Bounds must be chronological and year-0-free (the DateAxis
        ...
        def extract_place(provenience)
          name = AxisBuilder.normalize(provenience)
