...
module Nabu
  module Store
    module AxisBuilder
      # IIP dating + findspot → the date/place axis (P30-6) — the
      ...
      #   A year-0 bound is the DateAxis tripwire: skipped, counted
      #   invalid, never stored. The Periodo @period URIs stay in
      #   document metadata (no axis column for them).
      ...
      module IipDates
        ...
        # Walk the canonical records, insert one axis row per dated/
        ...
        def insert_row(catalog, document_id, path, counts)
          ...
          AxisBuilder.insert_axis(catalog, document_id, axis, IIP_SLUG)
          counts[:documents] += 1
        end
        ...
        # AxisBuilder's envelope policy over the plain attributes: min of
        ...
        def envelope(dates)
          ...
          dates.each do |node|
            ...
            raw ||= AxisBuilder.normalize(node.text)
          end
          {
        ...
        def extract_place(doc)
          ...
          AxisBuilder.normalize(own) || AxisBuilder.normalize(origin.at_xpath("./region")&.text)
        end
      end
    end
  end
