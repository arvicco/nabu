...
module Nabu
  module Store
    module AxisBuilder
      # I.Sicily dating + findspot → the date/place axis (P29-4) — the
      ...
      #   century CE"). Year 0 ("-0000", 1 record) is the DateAxis
      ...
      module IsicilyDates
        ...
        def insert_row(catalog, document_id, path, counts)
          ...
          AxisBuilder.insert_axis(catalog, document_id, axis, ISICILY_SLUG)
          counts[:documents] += 1
        end
        ...
        # AxisBuilder.envelope's policy over the -custom dialect: min of
        ...
        def extract_place(doc)
          ...
          name = AxisBuilder.normalize(ancient&.text)
          name ||= AxisBuilder.normalize(modern&.text)
