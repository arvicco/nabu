...
module Nabu
  module Store
    module AxisBuilder
      # EDH dating + findspot → the date/place axis (P17-2, edh-survey §4.1/
      # §4.2) — the largest single axis feed since HGV: 60,474 dated records
      ...
      #   values ("71–130", "212–"). A literal year 0 is the DateAxis
      ...
      #   such, up to 1998) ingest verbatim — the axis does not editorialize.
      ...
      module EdhDates
        ...
        # document-grain axis row per dated/placed document. Returns
        ...
        def insert_row(catalog, document_id, row, counts)
          ...
          AxisBuilder.insert_axis(catalog, document_id, axis, EDH_SLUG)
          counts[:documents] += 1
        end

        # One row's axis fields; nil when it carries neither date nor place,
        # :invalid on the year-0 tripwire.
        def extract(row)
          begin
            not_before = DateAxis.parse_year(row["dat_jahr_a"])
            not_after = DateAxis.parse_year(row["dat_jahr_e"])
        ...
        def extract_place(row)
          name = AxisBuilder.normalize(row["fo_antik"].to_s)
          name ||= AxisBuilder.normalize(row["fo_modern"].to_s)
