...
module Nabu
  module Store
    module TimelineBuilder
      # Vienna wiki dating + findspot → the timeline (P29-3) — one
      ...
      #   counted, never Timeline-invalid.
      ...
      #   coordinates decision — the timeline has no coordinate columns).
      module ViennaWikiDates
        ...
        # Walk both wikis' canonical pages, insert timeline rows. Returns
        # { lexlep: { documents:, undated:, invalid: }, tir: { … } } —
        # +undated+ counts joined inscriptions with a place-only row or no
        # row, +invalid+ the Timeline year tripwire (unreachable for the
        ...
        # One inscription's timeline fields via its cached Object page; nil
        # when it carries neither date nor place (or the object page is
        # not cached — an honest absence), :invalid on a Timeline-rejected
        # year.
        def extract(root, path)
          ...
          place = TimelineBuilder.normalize(object["site"])
          return nil if year.nil? && place.nil?

          date_raw = TimelineBuilder.normalize(object["date"])
        ...
        # else goes through the strict Timeline parse.
        def sort_year(raw)
          value = raw.to_s.strip
          return nil if value.empty? || value.match?(/\A-?0+\z/)

          Timeline.parse_year(value)
        end
        ...
        def read_wikitext(path)
          ...
          nil # a broken page costs its timeline row, never the build
        end
