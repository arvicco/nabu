...
module Nabu
  module Store
    module TimelineBuilder
      # RIIG dating + findspot → the timeline (P25-1) — the EDH
      ...
      #   (TimelineBuilder.envelope — alternatives widen, never a midpoint).
      #   precision: HGV's explicit @precision never appears here, so a
      #   point reads "exact" and a bounded interval "range"; date_raw = the
      #   record's own display text ("-Ier siècle"). Year 0 is the Timeline
      ...
      module RiigDates
        ...
        def insert_row(catalog, document_id, path, counts)
          ...
          TimelineBuilder.insert_timeline(catalog, document_id, timeline, RIIG_SLUG)
          counts[:documents] += 1
        end
        ...
        def extract(xml)
          ...
          begin
            bounds = TimelineBuilder.envelope(doc.xpath("//origin/origDate"))
        ...
        def extract_place(doc)
          ...
          name = TimelineBuilder.normalize(findspot&.text)
          name ||= TimelineBuilder.normalize(settlement&.at_xpath("./placeName")&.text)
