...
module Nabu
  module Store
    module TimelineBuilder
      # ORACC catalogue dates (P16-3, part 2 of the timeline). Every
      ...
      # (…:<textid>-en, P13-4) carries its tablet's timeline row: the date/place
      ...
      module OraccDates
        ...
        # one document-grain timeline row per joined document. Returns
        ...
        def build(catalog:, canonical_dir:)
          ...
          catalogue_paths(root).each do |path|
            ...
            data.fetch("members", {}).each do |textid, member|
              ...
              timeline = extract(member)
              undated += ids.size if timeline.nil? || timeline[:not_before].nil?
              next if timeline.nil?

              ids.each { |id| TimelineBuilder.insert_timeline(catalog, id, timeline, "oracc") }
              documents += ids.size
            end
          end
        ...
        # One member's timeline fields, or nil when it yields neither date nor
        ...
        # year-0 tripwire: unparseable, never stored (Timeline invariant).
        ...
        # Timeline century bounds the --century flag uses (BCE century -N).
        def parse_centuries(raw)
          ...
          not_before = Timeline.century_bounds(-indices.max).first
          not_after = Timeline.century_bounds(-indices.min).last
        ...
        def extract_place(member)
          name = TimelineBuilder.normalize(member["provenience"].to_s)
