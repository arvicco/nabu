...
module Nabu
  module DataBuild
    ...
    # (P51-W5): unigram-cost Viterbi over tsheg-bar units with a closed
    # attached-clitic class. Tsheg (་) delimits SYLLABLES, not words, so the
    # segmenter's job is twofold: MERGE syllable runs into multi-syllable
    # words (སངས་རྒྱས), and SPLIT the attached clitics that hide inside one
    # tsheg-bar (བླ་མའི = བླ་མ + genitive འི; དཔེར = དཔེ + terminative ར).
    #
