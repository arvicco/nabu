...
4. Implement `lib/nabu/adapters/<source>.rb`, composing an existing parser family if one fits. If a new parser family is needed, it gets its own class + tests first. **If `#fetch` materializes anything into `canonical/<slug>/` beside upstream's tree** (a tarball extraction, a pinned sidecar file), **declare it in `self.materialized_paths`** (Q59-a) — undeclared artifacts make the tree weak-identity and the source re-parses on every incremental rebuild.
...
- Long-running/network operations (real syncs, fetches, re-parses, fixture refresh) are LOOP-executed (owner rule 2026-09-14, superseding the original human-initiated stance): adopting a packet or phase authorizes its network operations, and the loop runs them — announced, ticked, and verified — rather than handing the owner commands. The owner's desk keeps only decisions and identity acts. Claude Code runs `rake test` and `rake lint` freely.
