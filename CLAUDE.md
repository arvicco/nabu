...
- **Never touch `canonical/` from application code except through `Adapter#fetch`, the ad-hoc pipeline, and a local shelf's one sanctioned write gateway (`Nabu::LanguageShelf` for dossiers, `Nabu::LibraryShelf` for the library — the fetch analogues for authored/acquired-not-downloaded data, driven by `nabu ingest`; architecture §16).** Canonical data is the permanent asset. Loader and enrichers are read-only on it.
