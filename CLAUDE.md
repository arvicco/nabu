...
- **Never touch `canonical/` from application code except through `Adapter#fetch`, the ad-hoc pipeline, and a local shelf's one sanctioned write gateway (`Nabu::LanguageShelf` — the fetch analogue for authored-not-downloaded data; architecture §16).** Canonical data is the permanent asset. Loader and enrichers are read-only on it.
