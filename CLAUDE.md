...
- **External communications follow `.docs/external-communications.md`.** All outbound email (licensing/permission asks, clarifications, thank-yous) is governed by that protocol: the mailbox is Fastmail via its MCP server (`mcp__fastmail__*`, token in `local/config/settings.yml`), **I draft as thread replies, the owner sends every email** (identity act — the token deliberately lacks the send scope). The thread index is `.docs/email-register.md` (subject + exchange summary + status; no email texts). **The inbox check rides the phase gate** (like `site:refresh`): at every gate — and whenever asked — read replies in tracked threads, update the register, and prepare any new drafts. Alert the owner to any draft unsent for **2+ days** in the turn's OWNER ACTIONS.
