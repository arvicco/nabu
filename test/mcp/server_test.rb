...
module MCP
  ...
  class ServerTest < Minitest::Test
    ...
    def test_tools_list_exposes_the_three_tools_with_schemas
      initialize!
      response = roundtrip(request("tools/list", {}, id: 2))
      tools = response.dig("result", "tools")
      assert_equal(%w[nabu_search nabu_show nabu_status], tools.map { |t| t.fetch("name") })
      tools.each do |tool|
    ...
    def test_tools_list_tolerates_absent_params
      ...
      assert_equal 3, response.dig("result", "tools").size
    end
