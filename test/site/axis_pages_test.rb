...
class AxisPagesTest < Minitest::Test
  ...
  # The desk-list heading, derived from the SAME census the generator words
  # it from (P45-2 recalibration: the literal "twenty" went stale the moment
  # the twenty-first axis was minted — era-bound census claims derive, never
  # freeze).
  def desks_heading
    count = axes.each_axis.count
    word = Nabu::Ops::AxisPages::WORD_NUMBERS.fetch(count) { count.to_s }
    "## The #{word} desks"
  end

  # [[name, block-body], …] split on the ### axis headings under the desk
  # heading.
  def index_blocks
