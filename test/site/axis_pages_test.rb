...
class AxisPagesTest < Minitest::Test
  ...
  # twenty-one desks".
  def index_blocks
    section = index_body[/^## The twenty-one desks\n(.*?)(?=^---\s*$)/m, 1]
    refute_nil section, "the /axis/ index must carry a '## The twenty-one desks' section"
    section.scan(/^### (.+?)\n(.*?)(?=^### |\z)/m)
  end
  ...
  def test_index_carries_the_quickstart_pointer_before_the_desk_list
    ...
    assert body.index(QUICKSTART_POINTER) < body.index("## The twenty-one desks"),
           "the /axis/ index pointer must sit before the '## The twenty-one desks' heading"
  end
