# Timeline fixtures (P15-2 + P16-3)

Trimmed-real HGV EpiDoc metadata records for the TimelineBuilder extractor tests
(`test/store/timeline_builder_test.rb`). Retrieved 2026-07-12 from the local
...
Layout mirrors the canonical tree so `TimelineBuilder.rebuild!(canonical_dir: …)`
...
date shapes for `TimelineBuilder::OraccDates`:
...
divs at ≤2 sentences each for `TimelineBuilder::ChronicleAnnals`: `Introduction`
