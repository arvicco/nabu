...
Real, untrimmed upstream samples from the PerseusDL canonical repos
(CLAUDE.md fixture rules). All files were small enough to keep whole —
structurally intact beats trimmed.
...
## Editions (4) — chosen to cover the parser's hard cases
...
textgroup-level `__cts__.xml` (7 metadata files total) — the adapter resolves
titles, URNs, and edition-vs-translation from these.
