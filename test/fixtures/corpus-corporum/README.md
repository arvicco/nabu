...
## .corpus-corporum-fetch.json — the fetch checkpoint (P81-1)

The state file the schema-2 walk writes over exactly these catalog
responses — generated by driving `CorpusCorporumFetch.sync!` against the
`catalog/` and `texts/` fixtures above (WebMock; 2026-08-21), so every
value is the real product of the real 2026-08-19 upstream bytes. Carries
the P81-1 dating capture: per-author born/died (2028 born 850; 2034
992–1050 via the certainty="between" died event; 2173 1100–1199) and
per-work work_composition years (892 / 1000 / 1130) — the adapter's
parse-time dating lanes read it as canonical/<slug> content.
