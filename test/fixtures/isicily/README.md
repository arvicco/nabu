...
| `ISic001206.xml` | grc | **P59-0 unsigned-BCE exemplar**: `notBefore-custom="0301" notAfter-custom="0200"` for raw "3rd century BCE?" — the sign dropped upstream; the axes repair negates on the raw's era signal (→ -301/-200). Non-empty en translation div (mints `-en`). |
