...
module Store
  # Nabu::Store::AxisBuilder::IsicilyDates (P29-4): I.Sicily origDate/
  # origPlace → the date/place axis, over the checked-in isicily fixture
  ...
  class IsicilyDatesTest < Minitest::Test
    ...
    def build!
      Nabu::Store::AxisBuilder::IsicilyDates.build(catalog: @db, canonical_dir: FIXTURES_ROOT)
    end

    def test_ce_range_with_modern_findspot_and_geonames_ref
      make_document("urn:nabu:isicily:isic000001")
      counts = build!
      row = axis_for("urn:nabu:isicily:isic000001")
    ...
    def test_bce_bounds_are_signed_years_with_pleiades_ref
      ...
      build!
      row = axis_for("urn:nabu:isicily:isic001510")
    ...
    def test_a_metadata_only_record_still_feeds_the_axis
      make_document("urn:nabu:isicily:isic020002")
      counts = build!
      row = axis_for("urn:nabu:isicily:isic020002")
