...
module Store
  # Nabu::Store::AxisBuilder::IipDates (P30-6): IIP origin/date +
  # origin/placeName → the date/place axis, over the checked-in iip
  ...
  class IipDatesTest < Minitest::Test
    ...
    def build!
      Nabu::Store::AxisBuilder::IipDates.build(catalog: @db, canonical_dir: FIXTURES_ROOT)
    end

    def test_ce_range_with_settlement_and_verbatim_raw
      make_document("urn:nabu:iip:abur0001")
      counts = build!
      row = axis_for("urn:nabu:iip:abur0001")
    ...
    def test_bce_bounds_are_signed_years
      ...
      build!
      row = axis_for("urn:nabu:iip:jeru0490")
    ...
    def test_a_dateless_record_gets_a_place_only_row_and_counts_undated
      ...
      counts = build!
      row = axis_for("urn:nabu:iip:caes0371")
