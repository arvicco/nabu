...
module Store
  # Nabu::Store::AxisBuilder::CdliDates (P31-2): the CDLI catalog's own
  # period-string year envelopes → the date/place axis, over the checked-in
  ...
  class CdliDatesTest < Minitest::Test
    ...
    def test_undated_fake_modern_row_keeps_its_place_and_counts_undated
      ...
      counts = build!
      row = axis_for("urn:nabu:cdli:p274853")
    ...
    def test_place_policy_over_the_real_rows
      ...
      assert_equal "Garšana (mod. uncertain)", axis_for("urn:nabu:cdli:p323717").fetch(:place_name)
      dated_only = axis_for("urn:nabu:cdli:p480562")
    ...
    def test_ascending_bc_range_is_invalid_never_stored
      Dir.mktmpdir do |dir|
        ...
        assert_nil axis_for("urn:nabu:cdli:p900006")
      end
    end
