...
module Store
  # Nabu::Store::AxisBuilder::ViennaWikiDates (P29-3): the inscription →
  ...
  class ViennaWikiDatesTest < Minitest::Test
    ...
    def build!
      Nabu::Store::AxisBuilder::ViennaWikiDates.build(catalog: @db, canonical_dir: FIXTURES_ROOT)
    end
    ...
    def test_sortdate_point_year_with_site_place
      ...
      counts = build!
      row = axis_for("urn:nabu:lexlep:ao-1.1")
    ...
    def test_sortdate_zero_is_the_wikis_unknown_filler_never_a_year
      ...
      counts = build!
      row = axis_for("urn:nabu:lexlep:bg-1")
    ...
    def test_tir_joins_through_the_slash_bearing_site
      ...
      counts = build!
      row = axis_for("urn:nabu:tir:bz-10.1")
