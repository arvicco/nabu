...
module Store
  # Nabu::Store::TimelineBuilder::CopticScriptoriumDates (P17-1): manuscript
  ...
  class CopticScriptoriumDatesTest < Minitest::Test
    ...
    def build!
      Nabu::Store::TimelineBuilder::CopticScriptoriumDates.build(catalog: @db, canonical_dir: FIXTURES_ROOT)
    end
    ...
    def test_place_only_headers_get_a_row_with_null_bounds
      ...
      refute_nil row, "an undated but placed manuscript still anchors the place timeline"
    ...
    # adapter's for every fixture document, or timeline rows silently detach.
    def test_extractor_urns_match_the_adapter_minting
      adapter_urns = Nabu::Adapters::CopticScriptorium.new
                                                      ...
                                                      .map(&:id).sort
      extractor_urns = []
      Nabu::Store::TimelineBuilder::CopticScriptoriumDates.tt_headers(
        File.join(FIXTURES_ROOT, "coptic-scriptorium")
      ) do |meta|
        extractor_urns << Nabu::Store::TimelineBuilder::CopticScriptoriumDates.document_urn(meta)
      end
