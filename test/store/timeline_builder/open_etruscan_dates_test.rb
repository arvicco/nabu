...
module Store
  # Nabu::Store::TimelineBuilder::OpenEtruscanDates (P29-0): corpus
  ...
  # here; the upstream "0.0" bound (no year 0 exists) is the Timeline
  ...
  class OpenEtruscanDatesTest < Minitest::Test
    ...
    def build!
      Nabu::Store::TimelineBuilder::OpenEtruscanDates.build(catalog: @db, canonical_dir: FIXTURES_ROOT)
    end

    def test_bce_positive_years_flip_to_signed_historical_years
      make_document("urn:nabu:open-etruscan:cr-2.20")
      counts = build!
      row = timeline_for("urn:nabu:open-etruscan:cr-2.20")
    ...
    def test_dated_unplaced_rows_get_a_date_only_row
      ...
      build!
      row = timeline_for("urn:nabu:open-etruscan:ve-6.2")
    ...
    def test_the_year_zero_bound_is_the_tripwire_but_the_place_survives
      ...
      counts = build!
      assert_nil timeline_for("urn:nabu:open-etruscan:etp-313"),
                 "100.0–0.0 with no findspot: invalid date, no place — no row"
      row = timeline_for("urn:nabu:open-etruscan:etp-240")
    ...
    def test_en_siblings_never_join
      ...
      build!
      assert_equal 0, @db[:document_axes].count,
                   "sibling urns are not minted from row ids — no timeline row"
    end

    def test_findspot_conflicts_resolve_first_wins
      places = Nabu::Store::TimelineBuilder::OpenEtruscanDates.findspot_places(FIXTURES_ROOT)
