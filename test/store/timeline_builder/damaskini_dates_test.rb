...
module Store
  # Nabu::Store::TimelineBuilder::DamaskiniDates (P23-1): manuscript dates and
  ...
  # These are witness (copying/printing) dates — the right timeline for a
  ...
  class DamaskiniDatesTest < Minitest::Test
    ...
    def build!
      Nabu::Store::TimelineBuilder::DamaskiniDates.build(catalog: @db, canonical_dir: FIXTURES_ROOT)
    end
    ...
    def test_en_siblings_never_get_timeline_rows
    ...
    # fixture set, or timeline rows silently stop joining.
    def test_extractor_urn_mint_matches_the_adapter
      ...
      extractor_urns = Dir.glob(File.join(FIXTURES_ROOT, "damaskini", "tsv", "**", "*.txt"))
                          .map { |path| Nabu::Store::TimelineBuilder::DamaskiniDates.document_urn(path) }
                          .sort
