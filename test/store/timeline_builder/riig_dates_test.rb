...
module Store
  # Nabu::Store::TimelineBuilder::RiigDates (P25-1): RIIG origDate/origPlace →
  # the timeline, over the checked-in riig fixture records
  ...
  class RiigDatesTest < Minitest::Test
    ...
    def build!
      Nabu::Store::TimelineBuilder::RiigDates.build(catalog: @db, canonical_dir: FIXTURES_ROOT)
    end

    def test_bce_range_with_findspot_and_trismegistos_ref
      make_document("urn:nabu:riig:ahp-01-01")
      counts = build!
      row = timeline_for("urn:nabu:riig:ahp-01-01")
    ...
    def test_fr_siblings_never_join
      ...
      build!
      assert_equal 0, @db[:document_axes].count,
                   "sibling urns are not minted from filenames — no timeline row"
    end
