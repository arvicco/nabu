...
| `sl` | Bohorič long s ſ→s | The historical-Slovene fold (P13-9). goo300k/IMP passage text is the pristine Early Modern print surface, where non-final s is set as ſ (U+017F): "ſvoje", "dvanajſt", "oblaſt". The generic fold does NOT touch it — ſ is already lowercase, carries no combining mark, and plain `downcase` leaves it alone (only Unicode FULL case folding maps ſ→s) — so without this rule every ſ-bearing word is unfindable by any modern query. Exactly the grc ς→σ situation: one letter, two positional glyphs; Unicode's own case-folding table agrees. `tr`, length-preserving. Bohorič digraphs (zh=č, ſh=š) are deliberately NOT rewritten — that is orthographic modernization (the corpora's own `<reg>` layer, an annotation), never a fold; haček letters (č/š/ž) fall to the generic mark strip on both sides. The rule landed BEFORE any sl corpus was synced (goo300k/imp both `wired: false`), so the rebuild-storm caveat below was satisfied vacuously. |
| `gmh` | long s ſ→s | The Middle High German fold (P40-5), the `sl` precedent verbatim: ReM's stored passage text is the DIPLOMATIC manuscript layer, which writes non-final s as ſ (U+017F) throughout ("ſtet", "ſin", "erſterben" — fixture census, M058/M218B), and plain `downcase` leaves U+017F alone, so every ſ-bearing MHG word would be unfindable by a modern query. `tr`, length-preserving. The combining letters above (uͦ = u + U+0366, muͦzic → `muzic`) fall to the generic Mn strip; the corpus's own normalized layer (`@norm`) is an annotation, never a fold. The rule lands BEFORE any gmh corpus is synced (rem `wired: false`), so the rebuild-storm caveat below is satisfied vacuously. |
| `gml` | ReN lemma notation: homograph superscripts ¹–⁷ (with variant tails ²A/¹ᵃ)→dropped; superscript vowels ᵉ/ⁱ→dropped; °, *, α/β, = →dropped; ()/[] brackets→dropped (letters stay) | The Middle Low German fold (P46-5), argued from a census of ALL 1,333,166 non-null anno-layer lemmas in the ReN 1.1 deposit, not assumed: the lemma citation forms carry homograph indices ("wēsen²" ×386K for ², ×801K for ¹), the ° proper-name mark ("marcus°" ×31K), Lm superscript vowels ("êⁱn¹" ×94K) and optional-letter brackets ("went(e)" ×49K) that all survive the generic fold — an ASCII typist's "wesen" could never reach the gold lemma without this rule. Long vowels (ē, ê, ȫ, ü̂) fall to the generic Mn strip; the diplomatic SURFACE carries no long ſ (censused: zero corpus-wide, unlike gmh) and its combining letters (ghoͤnen, tuͦ) fall to the generic strip too. Comma-chained lemma sets ("dê¹,dê¹,dat²A" → `de,de,dat`) stay chains — upstream's own single tag value, a documented lemma-search limitation. The rule lands BEFORE any gml corpus is synced (ren `wired: false`), so the rebuild-storm caveat below is satisfied vacuously. |
| `ara`, `fas` (one shared rule) | alef wasla ٱ→ا; alef maqsura ى→ي; farsi yeh ی→ي, keheh ک→ك; taa marbuta ة→ه; ZWNJ (U+200C)→space; tatweel (U+0640)→dropped | The Arabic-script fold (P41-3), censused against the OpenITI fixtures. **What the generic fold already does** (pinned in test, not re-ruled): the hamza-carrying alefs أ/إ/آ/ؤ/ئ canonically DECOMPOSE (أ = ا + U+0654), so NFD + `\p{Mn}` strip folds them bare; tashkeel (U+064B–0652), the superscript alef (U+0670) and the Persian ezafe hamza (بقیهٔ) are Mn and strip the same way — no rule needed for packet items 1–2 beyond wasla. **Maqsura ى→ي, ruled on fixture evidence:** the SAME Arabic files spell the same word both ways — فى ×69 vs في ×53, and the 0646 metadata writes the author's own nisba الخونجى ×2 AND الخونجي ×1 — upstream is internally inconsistent (Shamela's undotted final yeh), so the fold is mandatory; it also completes the cross-keyboard triangle (Persian علی → علي reaches Arabic على). **The cross-language pair (the P41-g census key finding):** Persian text carries ONLY ی (U+06CC) and ک (U+06A9); Arabic text ONLY ي (U+064A) and ك (U+0643) — internally consistent per language, so both collapse to the Arabic skeleton (the lower codepoints; an index form, never display) and a Persian-keyboard query finds Arabic text and vice versa, fold-both-sides. **Taa marbuta ة→ه:** standard Arabic-search practice (Lucene's ArabicNormalizer does exactly this), and the Persian IbnSina fixture itself writes the Arabic loan وولاة with ة inside Persian text where native Persian orthography writes ه — one corpus, both spellings. **ZWNJ→space**, the cuneiform precedent: each member of a Persian compound becomes its own searchable token (می‌دانی → `مي داني` — the stem داني findable alone; Lucene's PersianCharFilter practice); separator runs are FTS-invisible; trade-off accepted and documented: a fused-typed query (میدانم) does not match the ZWNJ-written corpus. **Tatweel dropped** (Lm — downcase and the Mn strip both leave it): pure elongation, never phonemic; fixture bytes: the Hijri marker هـ (a 1→0 `delete`, `fold_with_map`-safe). **Deliberate NON-rules, measured:** Arabic-Indic digits (٠١٢٣ U+0660–0669) and extended digits (۰۱۲۳ U+06F0–06F9) censused ×0 across all six fixtures — the fused footnote digits (حديث2) are ASCII already — rules on evidence, not symmetry (the `gmw` precedent; a one-place change here if a digit-bearing corpus lands); the Persian-only letters پ چ ژ گ stay (no Arabic counterpart — folding ک covers the shared letter, not these); wasla ٱ is itself censused ×0 in the fixtures and carried packet-directed for the Quranic orthography the corpus at large holds. Per-codepoint `tr` (1→1) plus the tatweel `delete` (1→0), `fold_with_map`-safe. The rule lands BEFORE any ara/fas corpus is synced (the openiti adapter is concurrent P41 work, `wired: false`), so the rebuild-storm caveat below is satisfied vacuously. |
| `egy` | Egyptological alef ꜣ→`a`, ain ꜥ→`a`; ʾ→dropped | The Egyptological-transliteration fold (P28-1), argued from a census of all 35,052 AED headwords, not assumed: ꜣ (U+A723, ×12,753; uppercase Ꜣ U+A722 ×284 reaches the rule via downcase) and ꜥ (U+A725, ×6,451; Ꜥ U+A724 ×357) are base letters with **no decomposition** — the generic `\p{Mn}` strip and downcase leave them, and no ASCII keyboard carries them — so ꜣ-/ꜥ-bearing headwords would be paste-only. Folded to `a`, the vocalic reading Egyptologists type in informal transliteration (ꜥꜣ → `aa`, `define aa` lands). ʾ (U+02BE MODIFIER LETTER RIGHT HALF RING, ×1,036) is category **Lm** — exactly the proto-superscript situation — and is dropped entirely (`gsub` 1→0, `fold_with_map`-safe; no ASCII typist spells it: ḥꜣp-rʾ → `hap-r`). Deliberate NON-rules, all measured: every dotted/lined consonant (ḥ ḫ ẖ š ṯ ḏ ṱ and their capitals) decomposes and falls to the generic strip (ḥtp → `htp` needs nothing); the semivowel breve i̯ (U+032F) and macron-below (U+0331) are Mn; the yod is spelled `j` upstream (no yod codepoint attested → no rule); compound punctuation (`.` suffix dots, `-` joins, the `=` clitic marker) is kept — it is text a user can type (`hw.t-ka`); the single editorial 〈 〉 pair (×1 corpus-wide) stays paste-only, one entry is not a rule. Keyed by primary subtag, so the papyri `egy-Egyd` Demotic passages refold under it (same letter inventory); the rule lands with the shelf `wired: false`, but those two already-synced Demotic documents mean the rebuild-storm caveat below is satisfied by the next planned `nabu rebuild`, not vacuously. |
| `lzh`, `och` (one shared table) | Han variant fold: trad↔simp↔z-variant codepoints collapse to ONE traditional skeleton (說/説/说 → 說) | The Han variant fold (P37-2), derived from the HELD unihan shelf's Variants data — **kTraditionalVariant/kSimplifiedVariant/kZVariant ONLY**; the 4,063 semantic-variant lines are structurally excluded (㐀 and 丘 are different *words* that mean the same thing — folding them is a lie, the owner-agreed line, pinned by refutation test). Too big to hand-census like `Nabu::Cyrl` (6,050 pairs at Unicode 17.0.0), so the one-place table is a GENERATED lib file: `Nabu::Hani` (`rake fold:hani` regenerates from `canonical/unihan/Unihan_Variants.txt`, read-only; provenance header carries version 17.0.0, file date 2025-07-24, generation date, counts). Canonical form = the **traditional** codepoint — what kanripo/cbeta store. Resolution (deterministic, conservative; full rule on `Nabu::Ops::HaniFoldBuilder`): z-clusters union-found first; single-target trad folds direct (亚→亞, ×5,973); multi-target forgiven only when all targets are one z-cluster (说→{說,説}, both z-linked → cluster canonical 說, the lowest codepoint, ×8); multi-target across clusters REFUSED (发→發 *emit*/髮 *hair*: picking one is a guess, ×63 censused); a char LISTING ITSELF among its traditional targets is upstream saying it is its own traditional word — REFUSED (时/了/台/体/单, ×414 censused; folding 了→瞭 would merge two real words) — unless mutual kZVariant edges declare one abstract character (吴 self-lists but 吳/吴/呉 are one z-cluster → 吳); reverse kSimplifiedVariant evidence cross-checks the direct folds (zero trad/simp disagreements at 17.0.0 — the never-merge-across-disagreement rule had nothing to drop); anchorless z-pairs take the lowest codepoint (值→値 — arbitrary but stable, and invisible: the skeleton is an index form, never display); chains resolve to fixed points, and the one cycle in 17.0.0 (奨↔奬) resolves to its lowest codepoint — no table value is itself a key, asserted at generation. Scope: `lzh` (kanripo/cbeta/suttacentral) and `och` (Baxter-Sagart/TLS dictionary headwords fold-both-sides, so `define 说` reaches the 說 entry); deliberately NOT `zho` (the unihan shelf's own headwords stay byte-literal, the query union reaching both spellings anyway) and NOT applied wholesale to `jpn` — Japanese gets its OWN narrower reform fold (the `jpn` row below, P38-4), never this Chinese trad/simp table whose self-listing refusals (体 is a distinct Chinese word) and semantic merges are wrong for Japanese. Held-corpus census (canonical kanripo+cbeta+suttacentral-lzh bytes, 34,131 distinct Han codepoints; measured 2026-07-20 while the live catalog was mid-rebuild with lzh not yet loaded): 1,064 held codepoints re-fold as keys, 4,981 stand as canonical targets, 6,045 of 6,050 pairs touch held text; the conservative price, also measured: 395 held self-listing + 25 held multi-trad codepoints stay unfolded (query 时 does not find 時 — a curated per-char whitelist is a possible future ruling, argued then, not assumed now). Per-codepoint 1→1 (`String#tr`), `fold_with_map`-safe. **Consequence: this fold changed `text_normalized` for every lzh/och row — the rebuild-storm caveat below is LIVE (P36-1 fold fingerprint dirtied 2026-07-20; the owner schedules the ~13M-passage lzh re-derive).** |
| `jpn` (its own table) | Japanese kyūjitai↔shinjitai reform fold: old-form, new-form and merged spellings collapse to one skeleton so a query matches MODERN READING HABITS (國/国 → 國, 學/学 → 學, and the admitted merges 辨/瓣/辯 → 弁); `search --exact` opts out to glyph-literal | The Japanese reform fold (P38-4 + P38-r1), a GENERATED lib file `Nabu::Jpn` (`rake fold:jpn` regenerates from `canonical/unihan/Unihan_OtherMappings.txt` + `canonical/edrdg/kanjidic2/kanjidic2.xml.gz`, read-only; provenance header carries Unihan 17.0.0 / KANJIDIC2 2026-202 and per-lane counts) in **two lanes**. **Lane 1 (jinmeiyō):** HELD unihan **kJinmeiyoKanji** pointers — Unicode's 2010 name-kanji list, where a jinmeiyō kyūjitai of a jōyō character points at that shinjitai (`U+OLD kJinmeiyoKanji 2010:U+NEW`) — the authoritative 1:1 reform pairs (173). **Lane 2 (kanjidic2-jōyō, P38-r1):** KANJIDIC2 `<variant jis208/jis213>` cross-references mined under the owner's ruled policy — **fold only pairs whose target (new form) is jōyō-listed**, jōyō = KANJIDIC2 `<grade>` 1–6/8 **∩** Unihan **kJoyoKanji** (intersection, the strict reading). This lands the high-frequency non-name reform pairs lane 1 cannot reach — 學/学, 體/体, 醫/医, 觀/観. Variant kuten decode through the held JIS X 0213 table (`Nabu::Adapters::Jis0213`); **jis212 is REFUSED** — JIS X 0212 is a different standard the 0213 table does not cover (decoding 學's jis212 1-33-55 through it misreads 宋). **Merges are ADMITTED** (owner ruling 2026-07-21, superseding the P38-4 refuse-mold): when a jōyō new form has ≥2 old claimants they all fold onto its skeleton (辨/瓣/辯 → 弁); a NAMED, editable census of the flagship distinct-word merges (弁, 缶, 欠, 芸…) documents each lexical collapse. Still refused (censused): a non-jōyō old variant-linked to **2+ distinct jōyō** forms (碕 → 崎/埼, one-to-many ambiguity — never pick arbitrarily), NFC-identity drops, and jinmeiyō-lane conflicts (lane 1 wins). **Composition (compose, not fight):** a 1:1 pair folds onto the more-complex member's `Hani.fold` skeleton — the traditional the lzh/och lane stores (觀→観 onto 觀; a Chinese *simplification* like 弃 for 棄 folds the rare glyph onto the common one, never the reverse); a merge folds onto the shinjitai's skeleton (the olds are distinct Chinese words the lzh lane keeps apart, so `Hani.fold(辨) ≠ Hani.fold(瓣)` — the Chinese lane is untouched). jpn passages apply ONLY this fold, never the whole Han table; the query union ORs both lanes. **The exact-match escape hatch:** `search --exact` runs the folded search for candidates then keeps only passages whose stored text carries the query glyph-literally (NFC-normalized, no case/diacritic/reform fold) — so 弁 tells apart from 辨/瓣/辯 when needed. Census: **744 fold entries** (173 jinmeiyō + 341 kanjidic 1:1 + 79 merges over 185 olds), 2 one-to-many refused, 0 jinmeiyō conflicts, 57 NFC-identity dropped; the SEMANTIC `NEW`/`OLD` cross-reference (`Jpn.old_form`/`new_form`, the char card) stays the 173 authoritative jinmeiyō pairs only, since a kanjidic `<variant>` link is not a reliable "old form". Per-codepoint 1→1 (`String#tr`), `fold_with_map`-safe. Rebuild-storm caveat: aozora is `wired: false`, so today the fold change is **vacuous** (no jpn passage exists to re-derive); the owner schedules the re-derive once jpn is synced. |
...
| `xct`, `bod`, `otb` | Tibetan script→EWTS (`Nabu::Xct`, GENERATED — `rake fold:xct` compiles it from the authored `config/ewts/rules.csv`, the same table the `xct/wylie-fold` nabu-data dataset publishes; P50-W3) | The Tibetan desk spells one script two ways: the canon shelves (derge-kangyur/derge-tengyur/soas-tibetan `xct`, old-tibetan `otb`) and the `bod` dictionary headwords (wiktionary-bo, tibetan-verbs) store Tibetan script, while mvp's `bod-Latn` lane and every scholarly query type Wylie — without the fold, `define shar ba` could never reach ཤར་བ and a pasted བྱང་ཆུབ་སེམས་དཔའ found nothing in Wylie-side dictionaries. EWTS is positional, so this is a transcode, not a per-codepoint rule: the inherent *a* seats on the syllable's ROOT (found by the classical prefix/suffix grammar — དག = *dag* but འདས = *'das*, བསྐྱབས = *bskyabs* with the *a* after the whole stack), subjoined stacks concatenate (query-form grain: no EWTS `+` operator), the suffix a-chung carries the connective particles (དགའི = *dga'i*), a bare MEDIAL a-chung opens an inner syllable (ནའང = *na'ang*), and loan stacks fall back to per-stack *a* (པདྨེ = *padme*, ཀརྨ = *karma*). Genuinely ambiguous vowel-less three-letter syllables prefer the prefix reading with a curated override list (མངས = *mangs*, བགས = *bags*) — documented in the dataset README. Old Tibetan registers because the script is one: reversed gi-gu → `-i` (OTDO's own convention), da-drag rides the postsuffix rule. The transcoder emits spec-EWTS capitals (oM, Ta); the paired `LANGUAGE_FOLDS` xct/bod/otb case-fold lowers them symmetrically, since `search_form`/`query_forms` downcase before the neutralizer runs and a typed query can never carry the capital. Idempotent on EWTS/Latin text; unknown codepoints pass through (the Betacode stance). A rule change regenerates `lib/nabu/xct.rb` and dirties exactly the Tibetan-consulting sources (`DerivationFingerprint::FOLD_LANGUAGES`, the P39-1 scoped mechanism) — the rebuild-storm caveat below applies to the Tibetan shelves; the P50-W3 WIRING itself (normalize.rb + fingerprint registration) dirties every source once, the standing price of any wiring change. |
