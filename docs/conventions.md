...
| `jpn` (its own table) | Japanese kyūjitai↔shinjitai reform fold: old-form and new-form spellings of a reform pair collapse to the shared traditional skeleton (國/国 → 國, 廣/広 → 廣, 圓/円 → 圓) | The Japanese reform fold (P38-4), derived from the HELD unihan shelf's **kJinmeiyoKanji** pointers — Unicode's own list of the 2010 jinmeiyō (name-use) kanji, where a jinmeiyō character that is the kyūjitai (traditional variant) of a jōyō character carries a pointer to that jōyō (shinjitai) codepoint (`U+OLD kJinmeiyoKanji 2010:U+NEW`). Those pointer lines ARE the reform pairs: one authoritative, Unicode-declared, 1:1 old→new mapping. A GENERATED lib file like `Nabu::Hani`: `Nabu::Jpn` (`rake fold:jpn` regenerates from `canonical/unihan/Unihan_OtherMappings.txt`, read-only; provenance header carries version 17.0.0, file date 2025-07-24, counts). **Merge policy** (the 弁 question): the famous many-to-one reform merges — 辨/瓣/辯 → 弁, 豫/預 — are NOT in kJinmeiyoKanji (the merged-away old forms are not name-kanji), so the source is merge-free by construction; and the builder STILL refuses structurally — two old forms pointing at one new (or vice versa) drop both, censused, never picking one of 辨/瓣/辯 to be "the" traditional of 弁. **Composition with the Han fold** (compose, not fight): the canonical is not the raw kyūjitai but `Hani.fold(kyūjitai)` — the exact traditional fixed point the lzh/och fold assigns — so a Japanese old/new form lands on the SAME index skeleton as the Chinese trad/simp forms (國), and the two folds agree rather than compete; for the 7 pairs where Unihan makes the shinjitai the traditional (the 奨↔奬 cycle family: 奬 桧 禄 緖 搖 遥 郞) the OLD form folds instead, still onto the shared skeleton. jpn passages apply ONLY this fold (never the whole Han table); the query union still ORs both lanes, so cross-corpus findability (a `国` query reaching kanripo `國` and Aozora `国` alike) holds regardless. Census: 230 raw kJinmeiyoKanji pointers − 57 NFC-identity (CJK compatibility ideographs collapsing post-NFC) = **173 reform pairs / 173 fold entries** (7 hani-composed onto the shinjitai), 0 merges. **Coverage bound, stated honestly:** kJinmeiyoKanji only covers kyūjitai that are registered name-kanji, so the highest-frequency non-name reform pairs — 學/学, 體/体, 醫/医, 觀/観 — are OUT of scope and stay literal (a `search 學` will not reach Aozora's `学`). Deliberately NOT synthesised from KANJIDIC2's `<variant jis208/212/213>` cross-references, which are kuten-coded and one-to-MANY (學 lists both 学 and the itaiji 斈) with no clean signal for THE kyūjitai — resolving them would mean inventing a disambiguation policy the hani mold forbids. Per-codepoint 1→1 (`String#tr`), `fold_with_map`-safe. Rebuild-storm caveat: aozora is `enabled: false`, so today the fold change is **vacuous** (no jpn passage exists to re-derive); the owner schedules the re-derive once jpn is synced. |
