...
| 6 | **GRETIL** | Vast Sanskrit (+ Pali, Prakrit, Tibetan…) e-texts | TEI P5 XML (mass-converted), legacy HTML/text | 5 | 2 | 3 | 3 (CC BY-NC-SA 4.0, `nc`) | **Status: SCOUTED (P9-4a, 2026-07-08).** Current corpus = mass-converted TEI P5 (~784 Sanskrit texts, ~240 MB; GitHub mirror `mmehner/gretil-corpus-tei@master`, site cumulative `.zip` per language, Zenodo DOI snapshots). **NOT EpiDoc** — no `refsDecl`, no CTS, no CapiTainS: EpidocParser does **not** apply, this is a **new bespoke parser family** (opus). Heterogeneous addressability: a hand-crafted minority (e.g. `sa_Rgveda-edAufrecht.xml`) has real `div[@type/@n]` → `lg[@xml:id]` → `l[@n]`; the bulk is flat `<lg>/<l>` or `<p>` with verse numbers embedded **as text markers** (`// BrbUp_1 //`), no attribute citation; prose texts are flat `<p>` with **no addressable structure at all**. IAST throughout (`sa-Latn`), NFC-friendly; Vedic accents ride in `<choice><orig>…</orig></choice>`. License uniformly **CC BY-NC-SA 4.0** in every TEI header → `license_class: nc` — but the header also disclaims *"provided in good faith that no copyright rights have been infringed … removed pending resolution of the claim"* (GRETIL is an aggregator, not the rights-holder). NC → ingestable for local research, kept out of the P8-1 MCP surface and any redistribution regardless. Fixture plan → backlog P9-4a, **owner approval pending**. |
