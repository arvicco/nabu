...
|---|---|---|---|
| Holdings & census (documents, passages, entries, lemma rows) | The live catalog (`db/catalog.sqlite3` + `db/fulltext.sqlite3`), read via `nabu status` / `list` / `axis` / `language` | Every number in README, docs, and the site is a **dated snapshot** read from the catalog | Gate-time refresh (library.md §10); site numbers are *generated* — `rake site:data` writes `site/_data/census.yml` (live-catalog census, as_of-stamped) + `desks.yml` (registry projection), drift-guarded by `test/site/site_data_test.rb` |
...
| Languages (codes, curated context) | `local/shelves/local-language/` dossiers (edit in any editor; `nabu sync local-language` re-derives) → catalog `language_records`; derived names census `language_names` | `nabu language CODE`; [docs/languages.md](languages.md) (dated snapshot) | Gate-time refresh alongside library.md |
| Silver lemmas (model output, P84-1) | `local/shelves/local-lemmas/<language>/` JSONL shards (written ONLY by `nabu lemma-enrich` through `Nabu::LemmaShelf`; venv bootstrap: [docs/manual/silver-lemma-venv.md](manual/silver-lemma-venv.md)) | `passage_lemmas` tier-`silver` rows + `lemma_frequencies` (fulltext-side; №R-51-B, P87: projected ONLY on the owner's word — `lemma-enrich --index/--index-only`, cache-backed — a full rebuild never projects, and sync-refresh maintains only an already-armed index); `search --lemma` labels silver hits | `lemma-enrich --spot-check N` (the P79-4 gold protocol) guards model drift; a malformed shard quarantines at sync/rebuild; the derivability writers allowlist pins the gateway |
...
|---|---|---|
| Nightly | `nabu backup` (canonical/ + config/ + local/ + .docs/ — NOTHING under db/ is ever backed up, owner ruling 2026-08-16; rebuild is the restore path for db/) + `nabu verify` (hash canonical vs catalog) | launchd jobs (ops.md templates) |
| Weekly | `nabu sync --all` → `nabu health` → `nabu health --remote` | launchd or manual; health trend rules flag anomalies |
| Monthly *(optional)* | `rake fixtures:check` — upstream **format** drift the fixtures no longer match | launchd (ops.md templates) |
...
- **Owned clones, not submodules.** Git-based sources are cloned and pulled IN PLACE under `canonical/<slug>/` by `Nabu::GitFetch` (fetch objects → mass-deletion breaker → attic upstream deletions → ff-only merge; architecture §8) — no cache-then-copy, no per-sync canonical commit; the recorded upstream SHA is the ledger pin. Submodules would couple your repo's health to upstream force-pushes and renames; owned clones keep every sync explicit, diffable, and non-destructive.
...
4. **New query surfaces** — the MCP read-only surface shipped first (architecture §9; 13 tools as of P78, one hash entry + handler per addition); still ahead: the read-only JSON endpoint over Tailscale (Sinatra, mirrors the knowledge-pipeline pattern; eventually public at nabu.ac), then possibly a thin web UI. These consume the store and never gain write paths — preserves the rebuildability invariant.
...
6. **Is the right to *fetch* conveyed by a public license?** If NO — the upstream granted this project's author a *personal* permission (StarLing's e-mail grant; the future TITUS Avestan) that a public clone of nabu does **not** inherit — the source is **grant-gated**. Set `grant_required: true` and a `grant:` block on its `config/sources.yml` row (`grantor` / `date` (quote it — a bare `2026-07-15` is a YAML Date and is refused) / `terms` (a short verbatim quote) / `thread` / `request_hint` — whom a new user should ask and what to promise). `nabu sync <slug>` then prints the terms and requires a typed `granted` before the first fetch, recording the acknowledgment durably — `local/config/grants.yml` is written FIRST by `GrantGate#record!` and is the source of truth (restore needs only the config file); the ledger row is a historical mirror. `--grant-acknowledged` does the same non-interactively for scripted/owner use; `sync --all` and axis expansions **skip** an un-acknowledged grant source with one honest line, never prompting mid-batch. This is **distinct** from ordinary public-license obligations — CC BY-NC-SA needs no gate, the `license_class` machinery already carries attribution and the no-redistribution posture. The gate criterion is solely "the right to fetch is not conveyed by a public license." (P42-r1.)

This note *is* the adapter's spec and becomes the fixture-selection guide.

## 5. Schema and model evolution

- **Migrations forward-only**, one track per db (catalog/ledger/links/lects, each with its own `schema_info`), and `nabu rebuild` must work from any released schema version. The standing guards: the forward-only tracks themselves (an applied migration is never edited), `Ledger.open_with_lift!` (the one-shot pre-P7-1 catalog lift every write path runs on open), and the derivability manifest test — every store under db/ must name the rebuild stage that re-mints it.
...
- The **adapter conformance suite** is the contract's enforcement arm: when the contract gains a requirement (say, `script:` tag on passages), add it to conformance, watch every adapter fail, fix each — the suite converts a cross-cutting change from "audit every adapter by hand" (145 registry rows as of P78) into "make the tests pass."
...
- **Exit formats are first-class:** `nabu export` to plain text + JSONL must always work; `--format conllu` is DEFERRED until the enrichment phase (the CLI refuses it and its help says so — an honest absence, not a broken promise). The system's value survives even if the code doesn't.
