...
move — plus the mechanical, append-only manifest append); for the
local-source shelf it is `Nabu::SourceShelf` (P24-0, the third gateway) —
all driven by `nabu ingest`. Everything else stays read-only on the
shelves.
