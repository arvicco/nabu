...
`nabu rebuild --incremental` (P36-1) is an optimization OVER this invariant, never a relaxation of it: the full rebuild remains the reference semantics, and an incremental run must land content-equivalent to a fresh full rebuild of the same canonical tree (test-pinned: counts + content shas). Each replay stamps the source with its derivation fingerprint (`derivation_stamps`, in the catalog — dropped and re-stamped by every full rebuild, correct by construction); `--incremental` keeps the catalog, skips fingerprint-clean sources, and re-derives dirty ones through the same replay seam + per-source index refresh. A dirty incremental also re-derives the two P70 sibling dbs (the lect journal and the links instrument) exactly as the full rebuild does; a clean sweep skips them — the write-through CLIs keep both live, so only a hand-edited `lect_rulings.yml`/`link_scopes.yml` waits for the next full rebuild or dirty incremental. See §5 (derivation stamps bullet) for the fingerprint inputs and refusal rules. **Stage stamps (P87-3):** the per-source fingerprint is blind to *indexer-side* derivation changes, so each heavy fulltext-side stage additionally stamps its own derivation-version token (`stage_stamps`, minted by every full rebuild); `--incremental` compares and surgically re-derives exactly the stage a version bump dirtied against the kept catalog (a stale stage with no standalone corpus re-run — the composite fts/lemma pass — refuses loudly instead), `--trust-stages` mints stamps without re-deriving for an index known built by current code, and carrying stages through a TRUE full rebuild stays deliberately unsupported: a dropped catalog re-mints the row ids the carried tables would dangle on. **Trust-derivations (P89-1, №R-54):** the derivation-stamp counterpart — `--trust-derivations` re-stamps a source whose ONLY drift is the parser digest without replaying it, but only when a git-backed `Nabu::CodeVoucher` confirms the source's own parser closure predates the run's ELDEST stamp (never each stamp's own time: a long run stamps late from disk bytes while its rows come from code loaded at start, so a mid-run commit can beat a per-stamp horizon — the 2026-08-29 lying-stamp specimen); anything unvouchable replays honestly, never a run refusal. The same hazard is closed at the root for future stamps: both rebuild flavors WARM every source's code tokens before any replay (`DerivationFingerprint#warm`), so stamps describe the code the run loaded. **Builder carve-out (P89-1):** the corpus-wide timeline/facet builders live OUTSIDE the shared-core digest (the P39-1 carve-out pattern), covered by their own `builders_digest` stamped as the `__corpus-builders__` sentinel row; a builder-only edit re-runs the builders on the next incremental (even with zero dirty sources) instead of dirtying every per-source stamp. The silver-lemma projection is NOT a rebuild stage at all since №R-51-B: it applies only on the owner's word (`nabu lemma-enrich --index-only`, cache-backed, bare-table window when unarmed), and routine syncs maintain only an already-armed index.
...
enrichments(id, passage_id, kind, model, model_version, payload_json, at)
   -- meter scansions today (P44-7, §6); embeddings live in
   -- vectors.sqlite3 keyed by (model, urn) (P93-4)
...
- **The licence-agree click-through arm (`Nabu::LicenseAgreeFetch`, P80-7) automates the ONE POST-form case that is consent, not a wall.** PORTULAN CLARIN serves its artifact only to a Django form POST (GET the download URL → CSRF token + a licence-agree checkbox → the agreed POST streams the zip; no account, no captcha). Owner ruling №R-37 (2026-08-19) sanctions representing the owner's licence acceptance, and the mechanism's guardrail keeps that grant verbatim: the adapter DECLARES the licence string being accepted, the form's own hidden licence field must match it exactly, and any drift raises before a POST leaves the machine — it can never silently agree to terms nobody recorded. Shaped like the Faraday connection ZipFetch expects, so the whole staging/attic/state choreography above rides unchanged; upstreams that are still walls (captcha, accounts) stay on the ManualDrop arm.
