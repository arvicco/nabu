...
**Invariant:** data flows one way. Adapters/HTR produce canonical files; the loader derives SQLite from canonical files; enrichers write only to derived tables. catalog + fulltext + vectors = f(canonical): `nabu rebuild` proves it by regenerating them from `canonical/` byte-for-byte-equivalently (modulo enrichment, which replays from its own journal). **history.sqlite3 is an operational LOG** (P7-1, reclassified P70): an append-only ledger (run history, per-repo sync pins, license baselines, durable revision events) that is *never* derived and *never* dropped by rebuild — but under the P70 two-folder contract it is NOT part of the required backup: every owner DECISION it once held now lives in config/ (grants.yml, creep_acceptances.yml; lect rulings in lect_rulings.yml with the journal re-derived at rebuild), and losing the log on restore loses history, never data the library serves (№R-21). It is keyed by durable identity (source slug, repo url, passage/document urn), never by catalog row ids, which every rebuild re-mints.
